...
beta = args.beta
meters = np.zeros(6)
writer = SummaryWriter('runs/run_%s'%(now.strftime('%Y-%m-%d')),comment='Lr_%.5f_StepBeta_%.3f_Warmup_%d' % (args.lr,args.step_beta,args.warmup))
start_timer = timer()
...
for epoch in range(args.epoch):
    ...
    for batch in loader:
        ...
        if total_step % args.print_iter == 0:
            ...
            writer.add_scalar('Loss',progress_info['Loss'],total_step)
            writer.add_scalar('KL',progress_info['KL'],total_step)
            writer.add_scalars('Word/Topo/Assm', {'Word':progress_info['Word'], 'Topo': progress_info['Topo'], 'Assm':progress_info['Assm']},total_step)
            writer.add_scalar('Prop',progress_info['Prop'],total_step)
            writer.add_scalar('lr',progress_info['lr'],total_step)
            writer.flush()
            sys.stdout.flush()
            meters *= 0
            last_timer = timer()
